...
// Express passes listen failures into this callback (and also listens for 'error').
// If we ignore `err`, a taken port still prints "started" then the process exits 0 — no server handle.
app.listen(PORT, (err) => {
  if (err) {
    if (err.code === 'EADDRINUSE') {
      console.error(`\n  Port ${PORT} is already in use (another TrueGrant server or app).`);
      console.error(`  Stop that process, or use another port, e.g. PowerShell:`);
      console.error(`    $env:PORT=3002; npm run server\n`);
    } else {
      console.error(err);
    }
    process.exit(1);
